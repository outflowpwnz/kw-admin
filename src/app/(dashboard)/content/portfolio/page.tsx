...
import { portfolioApi, PortfolioCase } from '@/lib/api/portfolio'
...
export default function PortfolioPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<PortfolioCase | null>(null)
  ...
  const openCreate = () => {
    setEditing(null)
    reset({ title: '', photoUrl: '', description: '' })
    setPreviewUrl('')
    setDialogOpen(true)
  }

  const openEdit = (item: PortfolioCase) => {
    setEditing(item)
    reset({ title: item.title, photoUrl: item.photoUrl, description: item.description })
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true)
    try {
      const url = await portfolioApi.upload(file)
  ...
  const reorderMutation = useMutation({
    mutationFn: ({ id, direction }: { id: string; direction: 'up' | 'down' }) =>
      portfolioApi.reorder(id, direction),
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-3">
          {cases.map((item, i) => (
            ...
            >
              {/* Thumbnail */}
              <div className="w-20 h-14 rounded overflow-hidden shrink-0 bg-muted relative">
                {item.photoUrl ? (
                  <Image
        ...
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input {...register('title')} placeholder="Свадьба Ивана и Марии" />
              {errors.title && (
            ...
            <div className="space-y-1.5">
              ...
              {(previewUrl || watchedUrl) && (
                <div className="relative w-full aspect-video rounded overflow-hidden bg-muted mb-2">
                  <Image
            ...
            <div className="space-y-1.5">
              <Label>Описание (фишка свадьбы)</Label>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground line-clamp-2">{deleteTarget?.description}</p>
          <DialogFooter className="gap-2">
